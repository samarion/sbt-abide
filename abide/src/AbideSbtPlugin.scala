...
object AbideSbtPlugin extends AutoPlugin {
   ...
   * AbidePlugin
   *
   * Attempt at unifying management of local abide plugins (sbt sub-projects typically) and published
   * abide plugins (library dependencies) for the abide user.
   * 
   * Both libraries and projects can be added to the `abidePlugins` key and the plugin should deal with
   * these correctly internally to make usage as simple as possible for adopters.
   */
  sealed abstract class AbidePlugin
  implicit final class AbideArtifactPlugin private[abide] (val moduleID: ModuleID) extends AbidePlugin
  final class AbideProjectPlugin private[abide] (val project: Project) extends AbidePlugin

  /**
   * Abide plugin version (which determines the whole library version). This is specified in the project/Build.scala
   * file, and I don't know exactly how else to pick it up.
   */
  ...
  private lazy val abideSettings: Seq[sbt.Def.Setting[_]] = Seq(
     ...
     * The sbt API seems to require a ProjectRef here and not a Project so I don't now how best
    ...
    buildDependencies := {
      val projectPlugins = abidePlugins.value.collect {
        case app: AbideProjectPlugin =>
          ...
          val ref = ProjectRef(app.project.base, app.project.id)
    ...
    /* Use the sbt dependency resolution system to resolve library dependencies.
    ...
    libraryDependencies ++= Seq(
      "com.typesafe" % ("abide_" + scalaVersion.value + "_" + version) % version % AbideConfig.name,
